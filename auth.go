...
func (s *AuthService) login(u *auth.User) (authTokens, error) {
...
func (s *AuthService) refreshToken(rt string) (refreshTokens, error) {
	...
	if !ok {
		return refreshTokens{}, &req.ErrorResponse{Status: http.StatusUnauthorized, Message: http.StatusText(http.StatusUnauthorized)}
	}
...
func (s *AuthService) getUserInfo(email string) (auth.User, error) {
	q := auth.New(s.DBCon)
